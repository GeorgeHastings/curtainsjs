...
export class Renderer {
  constructor({
    ...
    if (!this.gl) {
      this.gl =
        this.canvas.getContext("webgl", glAttributes) ||
        this.canvas.getContext("experimental-webgl", glAttributes);
    }
    ...
    if (frameBuffer) {
      ...
      if (bufferId !== this.state.frameBufferID) {
        ...
        this.gl.viewport(
          0,
          0,
          frameBuffer._size.width,
          frameBuffer._size.height
        );
  ...
  removeRenderTarget(renderTarget) {
    ...
    let hasPlane = this.planes.find(
      (plane) =>
        plane.type !== "PingPongPlane" &&
        plane.target &&
        plane.target.uuid === renderTarget.uuid
    );
    ...
    for (let i = 0; i < this.planes.length; i++) {
      if (
        ...
        this.planes[i].target.uuid === renderTarget.uuid
      ) {
    ...
    this.renderTargets = this.renderTargets.filter(
      (element) => element.uuid !== renderTarget.uuid
    );
  ...
  removeShaderPass(shaderPass) {
    ...
    this.shaderPasses = this.shaderPasses.filter(
      (element) => element.uuid !== shaderPass.uuid
    );
